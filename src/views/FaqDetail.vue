<template>
  <div class="page-container" v-if="faq">
    <div class="page-title no-print">
      ...
      <el-button :icon="faqStore.isFavorite(faq.id)?StarFilled:Star" :type="faqStore.isFavorite(faq.id)?'warning':'default'" @click="faqStore.toggleFavorite(faq.id); fav=!fav">
        {{ fav ? '已收藏' : '收藏' }}
      </el-button>
      <el-button :icon="Edit" type="warning" @click="$router.push(`/faq/${faq.id}/edit`)">编辑</el-button>
    ...
    <el-row :gutter="20">
      <el-col :xs="24" :lg="18">
        <el-card shadow="never">
          ...
          <p class="faq-meta">
            <span><el-icon><Monitor /></el-icon> {{ machine?.name || '未知机型' }}</span>
          ...
          <div class="faq-tags" v-if="faq.tags && faq.tags.length">
            <el-tag v-for="tid in faq.tags" :key="tid" :color="faqStore.getTag(tid)?.color" effect="plain" style="border:none;margin:2px;cursor:pointer" @click="$router.push({ path:'/search', query:{ tag: tid } })">
      ...
      <el-col :xs="24" :lg="6" class="no-print">
        ...
        <el-card shadow="never" class="mb-16">
          ...
          <div v-if="relatedFaqs.length">
            <div class="related-item" v-for="r in relatedFaqs" :key="r.id" @click="$router.push(`/faq/${r.id}`)">
        ...
        <el-card shadow="never">
          <template #header><span style="font-weight:600">🏷️ 关键词</span></template>
          <div v-if="faq.keywords">
            <el-tag v-for="kw in faq.keywords.split(',').map(s=>s.trim()).filter(Boolean)" :key="kw" size="small" effect="plain" style="margin:3px;cursor:pointer" @click="$router.push({ path:'/search', query:{ q: kw } })">
              {{ kw }}
            </el-tag>
          </div>
...
onMounted(() => {
  if (faq.value) {
    ...
    fav.value = faqStore.isFavorite(faq.value.id)
  }
})
...
<style scoped>
.faq-title { font-size: 24px; font-weight: 700; line-height: 1.4; margin-bottom: 12px; display: flex; align-items: center; }
...
.section h2 { font-size: 18px; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 2px solid #409eff; }
