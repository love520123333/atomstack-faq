  ...
  <div class="page-container">
    ...
    <el-card shadow="never" class="search-card mb-16">
      ...
      <el-row :gutter="12" style="margin-top:16px">
        <el-col :xs="24" :sm="6">
          <el-select v-model="optMachine" placeholder="机型" clearable style="width:100%">
        ...
        <el-col :xs="24" :sm="4">
          <el-select v-model="optCategory" placeholder="分类" clearable style="width:100%">
            <el-option v-for="cat in machineStore.categories" :key="cat.id" :label="cat.name" :value="cat.id" />
          </el-select>
        </el-col>
        <el-col :xs="24" :sm="4">
          <el-select v-model="optTag" placeholder="标签" clearable style="width:100%">
            <el-option v-for="tag in faqStore.tags" :key="tag.id" :label="tag.name" :value="tag.id" />
          </el-select>
        </el-col>
        <el-col :xs="24" :sm="3">
          <el-select v-model="optPriority" placeholder="优先级" clearable style="width:100%">
        ...
        <el-col :xs="24" :sm="3">
          <el-select v-model="optStatus" placeholder="状态" clearable style="width:100%">
    ...
    <div class="result-list" v-if="results.length">
      <el-card shadow="hover" class="result-card" v-for="item in pagedResults" :key="item.id" @click="$router.push(`/faq/${item.id}`)" style="cursor:pointer">
        <div class="result-header">
          ...
          <el-tag size="small" type="info">{{ machineStore.getMachine(item.machineId)?.name || '未知' }}</el-tag>
          <el-tag v-for="tid in (item.tags||[]).slice(0,3)" :key="tid" size="small" effect="plain" style="margin-left:4px">{{ faqStore.getTagName(tid) }}</el-tag>
        </div>
...
onMounted(() => {
  ...
  if (route.query.tag) optTag.value = route.query.tag
  if (route.query.category) optCategory.value = route.query.category
  if (route.query.machine) optMachine.value = route.query.machine
  if (query.value || optTag.value) doSearch()
})

const results = computed(() => {
  if (!searched.value) return []
  return faqStore.searchFaqs(query.value || '.*', {
    machineId: optMachine.value || undefined,
    categoryId: optCategory.value || undefined,
    tagId: optTag.value || undefined,
    priority: optPriority.value || undefined,
    status: optStatus.value || undefined,
    sortBy: sortBy.value,
    machineStore
  })
})
...
function doSearch() { searched.value = true; currentPage.value = 1 }
function resetSearch() { query.value = ''; optMachine.value = ''; optCategory.value = ''; optTag.value = ''; optPriority.value = ''; optStatus.value = ''; searched.value = false }
</script>

<style scoped>
.mb-16 { margin-bottom: 16px; }
.search-card { background: #fafbfc; }
.result-card { margin-bottom: 12px; transition: transform 0.15s; }
...
.result-summary { color: #606266; font-size: 13px; margin: 8px 0; line-height: 1.6; }
.result-footer { display: flex; gap: 16px; color: #909399; font-size: 12px; }
.result-footer span { display: flex; align-items: center; gap: 4px; }
</style>
